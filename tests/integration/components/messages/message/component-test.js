...
module('Integration | Component | message', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.equal(
      document.querySelector('.message__user-name').innerText.trim(),
      'Lisa Huang-North',
      'Author name is ok'
    );
    assert.equal(
      document.querySelector('.message__timestamp').innerText.trim(),
      '12/02/2019, 07:31:14',
      'Timestamp is ok'
    );
    assert.equal(
      document.querySelector('.message__body').innerText.trim(),
